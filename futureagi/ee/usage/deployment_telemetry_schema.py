"""Shared wire contract for deployment telemetry (receiver copy).
...
**vendored verbatim** from the sender repo at
``tfc/deployment_telemetry/schema.py`` (only this module docstring, which
names each copy's role, differs). A contract test
(``test_schema_contract``) asserts every public constant in the two copies
agrees and that a payload built on the sender validates here, so a cap or
