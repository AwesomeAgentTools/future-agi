...
@pytest.mark.unit
def test_export_backfills_missing_start_time_to_queryable_partition(monkeypatch):
    # A pulled call with no createdAt/endedAt would otherwise emit a span with
    # no start_time; the collector writes that row into the 1970 partition of
    # CH `spans` (PARTITION BY toDate(start_time)), invisible to dashboards.
    # The export must always stamp a start_time (end_time -> now fallback).
    captured = {}
    ...
    span = SimpleNamespace(
        ...
        start_time=None,
        end_time=None,
        span_attributes={"raw_log": {"id": "c3"}},
    )
    op._export_provider_call_to_collector(span, "vapi", "c3")
    s = captured["spans"][0]
    assert "start_time" in s
    # Fallback to now() -> well within the current era, not 1970.
    assert s["start_time"] > 1_700_000_000_000_000_000


@pytest.mark.unit
def test_export_prefers_end_time_when_start_missing(monkeypatch):
    captured = {}
    ...
    end = datetime(2026, 6, 19, 12, 0, 30, tzinfo=UTC)
    span = SimpleNamespace(
        project=project,
        trace=SimpleNamespace(id=SimpleNamespace(hex="f" * 32)),
        name="Retell Call",
        ...
        start_time=None,
        end_time=end,
        span_attributes={"raw_log": {"id": "c4"}},
    )
    op._export_provider_call_to_collector(span, "retell", "c4")
    s = captured["spans"][0]
    assert "start_time" in s
    assert s["start_time"] == int(end.timestamp() * 1e9)
